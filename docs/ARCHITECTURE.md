...
기수 생성 트리거가 시작일을 `effective_from`으로 하는 최초 `penalty_rates` 행을 같은 트랜잭션에서 만든다. 계산 결과(완료/미제출/면제/스트릭/차감/반환액/크루 공동 달성률)는 저장하지 않고 원본에서 재계산한다.
...
- 참가자 화면에는 다른 구성원의 완료일·링크 수·연속 달성·순위를 반환하지 않고, 개인 성장 기록과 익명화한 크루 합계만 제공한다.
