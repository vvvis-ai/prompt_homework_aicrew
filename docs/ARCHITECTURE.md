...
명세의 Phase 1~8 순서를 유지한다. 각 Phase 종료 때 `typecheck`, `lint`, `build`, 핵심 Vitest를 실행한다. DB 연결이 제공되면 Supabase 마이그레이션·RLS 테스트와 DB advisor도 실행한다. 최종 산출물은 `.env.example`, 마이그레이션, seed, 운영 문서와 함께 배포 가능한 상태로 만든다.

## 매일 AI 실천 지원

- 참가자 홈의 `progress`는 월별 달력과 피드 필터에 독립적인 전체 챌린지 기간 집계입니다. `challenge-progress.ts`가 기존 숙제 판정 결과를 재사용해 개인 제출률과 익명 크루 합계를 계산합니다. 활동 그래프에는 현재 챌린지 참가자 전체의 날짜별 링크 수와 공유한 참가자 수만 반환하며, 다른 사람의 개별 상태·신원은 포함하지 않습니다.
- 활동 격자·주별 막대·월별 막대그래프는 KST 날짜별 크루 합계를 사용하며 누구를 선택해도 같습니다. 하루 여러 링크를 공유한 사람은 날짜별 참가자 수에서 한 명으로 셉니다. 홈의 크루 링크 수·활동일 수·날짜별 참가자 수는 마감 후(23:01부터), 주말, `excluded_dates`의 공휴일·운영 제외일 공유를 모두 제외합니다. 23:00:59.999까지는 기존 마감 판정을 재사용해 포함합니다. 숙제 제출률은 기존대로 완료와 확정 미제출만 분모로 사용합니다. 종료 기수의 자율 공유는 현재 기수 그래프에서 제외하며, 본인의 자율 공유 요약과 공유 피드 원본은 별도로 유지합니다.
- 제출·면제 조회는 페이지를 순회하여 누적 기록의 조회 한도로 인한 집계 누락을 방지합니다. 알림 카드는 참가자 홈의 마지막 콘텐츠로만 표시합니다.
- 이름 선택 후 홈 최상단과 성장 탭은 같은 `ChallengeDashboard`와 `progress.crew`를 사용합니다. 기존 `crewGrowth`의 제출률·공유 합계도 이 결과에서 가져와 조회 월에 따른 불일치를 방지합니다. 개인 기록만 월별 조회를 유지합니다.
- 누적 비율은 ‘확정 기준 제출률’로 표시합니다. `progress.crew.today`는 기존 날짜별 숙제 판정에서 오늘의 완료·대기·미제출·면제 인원만 익명 합산하며, 제출 대상은 완료+대기+미제출입니다. 면제·휴일·비참여자는 대상에서 제외하며, 시작 전·종료 후에는 대상 0명으로 표시합니다.

- `habits.ts`는 월 경계를 넘는 월~일 실천 기록과 최근 확정 미제출 연속 횟수를 계산합니다. 참가자 API에는 선택한 본인의 기록만, 관리자 API에는 운영용 기록을 제공합니다.
- 미션은 `daily_missions`에 기수·날짜별로 저장합니다. 운영진 인증을 거친 API에서만 등록·삭제하며 감사 기록을 남깁니다. 참가자에게는 활성 기수의 오늘 미션만 공개합니다. 미등록 날짜에는 결정적인 날짜 기반 기본 미션을 제공합니다.
- `push_reminders`의 endpoint와 기기 제어 토큰 해시는 서버만 읽습니다. 수정·해제는 기기에 저장된 임의 토큰을 검증하며, 푸시 발송 주소는 알려진 브라우저 푸시 서비스의 HTTPS 주소로 제한합니다.
- Cloudflare의 사용자 정의 Worker가 정기 실행을 받아 수명이 짧고 audience가 분리된 JWT로 내부 발송 핸들러를 호출합니다. 일반 관리자 세션은 이 핸들러에 사용할 수 없습니다.
- 전송 직전 기존 숙제 판정 함수를 재사용하고, 일별 발송 선점을 조건부 UPDATE로 처리합니다. 웹 푸시는 이름·금액·사유 없이 고정 문구만 전달합니다. 알림은 참여 여부를 증명하지 않으며 실제 학습 실천은 기존 링크 제출을 기준으로 표시합니다.
- 실서비스는 Cloudflare Workers + vinext로 배포하며, 위 초기 설계의 Vercel 런타임 언급보다 `docs/OPERATIONS.md`와 `wrangler.jsonc`를 운영 기준으로 사용합니다.
