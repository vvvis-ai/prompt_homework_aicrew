...
- 활동 격자·주별 막대·누적 그래프는 KST 날짜별 크루 합계를 사용하며 누구를 선택해도 같습니다. 하루 여러 링크를 공유한 사람은 날짜별 참가자 수에서 한 명으로 셉니다. 홈의 크루 링크 수·활동일 수·날짜별 참가자 수는 마감 후(23:01부터), 주말, `excluded_dates`의 공휴일·운영 제외일 공유를 모두 제외합니다. 23:00:59.999까지는 기존 마감 판정을 재사용해 포함합니다. 숙제 제출률은 기존대로 완료와 확정 미제출만 분모로 사용합니다. 종료 기수의 자율 공유는 현재 기수 그래프에서 제외하며, 본인의 자율 공유 요약과 공유 피드 원본은 별도로 유지합니다.
