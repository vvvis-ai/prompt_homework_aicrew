...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: before, error: readError } = await ctx.db.from("challenges").select("id,name,start_date,end_date,penalty_start_date,default_fee,default_penalty,is_active").eq("id", id).single();
    ...
    }).eq("id", id).select("id,name,start_date,end_date,penalty_start_date,default_fee,default_penalty,is_active").single();
