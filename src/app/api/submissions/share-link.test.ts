...
describe("server-side share link enforcement", () => {
  it.each(["https://chatgpt.com/c/private", "https://example.com/conversation", "https://example.com/notebook", "https://example.com/notebooks/example"])("blocks direct create and edit API calls: %s", async (url) => {
  ...
  it.each(["https://notebooklm.google.com/notebook/example-id", "https://example.com/NOTEBOOK/example?authuser=0"])("allows notebook links in create and edit: %s", async (url) => {
