...
export function HabitOverview({ data }: { data: AdminData }) {
  ...
  function reminderLabel(reminder: AdminReminder | null) {
    if (!reminder) return "알림 기기 미등록";
    const time = reminder.time.slice(0, 5);
    if (reminder.lastDeliveryStatus === "sent") return `${time} 알림 발송 성공`;
    if (reminder.lastDeliveryStatus === "test_sent") return `${time} 등록 · 테스트 성공`;
    if (reminder.lastDeliveryStatus === "failed") {
      return `${time} 등록 · 최근 발송 실패${reminder.lastResponseStatus ? ` (HTTP ${reminder.lastResponseStatus})` : ""}`;
    }
    if (reminder.lastDeliveryStatus === "expired") return `${time} 등록 · 만료된 기기`;
    if (reminder.lastDeliveryStatus === "sending") return `${time} 등록 · 발송 확인 중`;
    if (reminder.lastDeliveryStatus === "legacy_claimed") return `${time} 등록 · 기존 발송 처리 기록`;
    return `${time} 알림 등록`;
  }
  async function sendTest(participantId: string, name: string) {
    ...
    try {
      ...
      const result = await response.json() as { sent?: number; failed?: number; expired?: number; demo?: boolean; error?: string };
      if (!response.ok) throw new Error(result.error ?? "테스트 알림 발송에 실패했습니다.");
      setMessage(result.demo
        ? "체험 모드에서는 테스트 알림을 보내지 않습니다."
        : `${name}님의 등록 기기 ${result.sent ?? 0}대에 테스트 알림을 보냈습니다.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "테스트 알림 발송에 실패했습니다.");
    } finally {
  ...
  return (
    <div className="grid gap-5">
      <section className="habit-admin-hero">
        <div>
          <p className="habit-eyebrow">꾸준함을 돕는 운영</p>
          <h2 className="mt-2 text-2xl font-black">좋은 결과보다, 오늘 한 번의 사용</h2>
          <p className="mt-2 text-sm text-slate-600">아래 실천 현황은 조회 월과 관계없이 이번 주 기준입니다.</p>
        </div>
      ...
      <section className="surface-card">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-black">오늘의 참여</h2>
          <button className="filter-button" onClick={() => void copy()}><Copy size={16} />미제출 안내문 복사</button>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          {[["all", "전체"], ["pending", "마감 전 미제출"], ["completed", "완료"], ["missed", "미제출 확정"]].map(([value, label]) => (
            <button key={value} className={`filter-button ${filter === value ? "filter-active" : ""}`} onClick={() => setFilter(value)}>{label}</button>
          ))}
        </div>
        <p className="mt-3 whitespace-pre-wrap text-sm text-blue-700" role="status">{message}</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((person) => (
            <div key={person.participantId} className="rounded-xl border border-slate-200 p-3">
              <div className="flex items-center justify-between gap-2">
                <strong>{person.name}</strong>
                <span className={`text-xs font-bold ${person.status === "completed" ? "text-emerald-700" : "text-slate-500"}`}>{labels[person.status]}</span>
              </div>
              <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
                <span className={`text-xs ${person.reminder?.lastDeliveryStatus === "failed" || person.reminder?.lastDeliveryStatus === "expired" ? "text-rose-700" : "text-slate-500"}`}>
                  {reminderLabel(person.reminder)}{person.reminder && person.reminder.deviceCount > 1 ? ` · ${person.reminder.deviceCount}대` : ""}
                </span>
                {person.reminder && (
                  <button
                    className="filter-button shrink-0 px-2 py-1 text-xs"
                    disabled={busyParticipantId === person.participantId}
                    onClick={() => void sendTest(person.participantId, person.name)}
                  >
                    <Bell size={13} />{busyParticipantId === person.participantId ? "발송 중" : "테스트"}
                  </button>
                )}
              </div>
            </div>
        ...
        {!visible.length && <p className="py-5 text-sm text-slate-500">해당하는 참가자가 없습니다.</p>}
      </section>
      <section className="surface-card">
        <h2 className="text-lg font-black">가볍게 안부를 물어봐 주세요</h2>
        <p className="mt-2 text-sm leading-6 text-slate-500">최근 숙제 대상일에 2회 이상 연속 미제출한 참가자입니다. 주말·휴일·면제일은 건너뛰며, 오늘 마감 전 대기는 포함하지 않습니다.</p>
        <div className="mt-4 flex flex-wrap gap-2">{needsHelp.map((person) => <span key={person.participantId} className="rounded-xl bg-amber-50 px-4 py-3 text-sm font-bold text-amber-900">{person.name} · {person.recentMisses}회 미제출</span>)}</div>
        {!needsHelp.length && <p className="mt-4 text-sm text-emerald-700">현재 안부 확인 대상이 없습니다.</p>}
        <p className="mt-4 text-xs text-slate-400">이 목록은 운영진에게만 표시됩니다. 링크 수는 AI 사용량이 아닌 제출 기록의 지표입니다.</p>
      </section>
    </div>
  );
}
