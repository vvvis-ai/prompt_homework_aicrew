...
export function ReminderCard({ participantId, demo }: { participantId: string; demo: boolean }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!/^(0[8-9]|1[0-9]|2[0-2]):[0-5][0-9]$/.test(time)) throw new Error("08:00~22:59 사이에서 시간을 선택해주세요.");
  ...
  return <section className="surface-card"><div className="flex items-center gap-2"><Bell size={19} className="text-blue-600" /><h2 className="font-black">잊지 않도록, 하루 한 번</h2></div><p className="mt-2 text-sm leading-6 text-slate-500">선택한 시간(한국 시간)에 이 기기로 알림을 받아요. 제출 완료·휴일·면제일에는 보내지 않아요.</p><div className="mt-4 flex flex-wrap items-center gap-2"><label className="sr-only" htmlFor="reminder-time">알림 시간</label><input id="reminder-time" className="date-input" type="time" min="08:00" max="22:59" step="60" value={time} onChange={(e) => setTime(e.target.value)} /><button className="filter-button" disabled={busy} onClick={() => void save()}>{busy ? "처리 중…" : saved?.participantId === participantId ? "알림 설정 저장" : "이 이름으로 알림 켜기"}</button>{saved && <button className="filter-button" disabled={busy} onClick={() => void disable()}>알림 끄기</button>}</div><p className="mt-3 text-xs leading-5 text-slate-500" role="status">{message || (saved?.participantId === participantId ? `알림 켜짐 · ${saved.time}` : saved ? "이 기기에 다른 이름의 알림이 설정되어 있어요. 켜기를 누르면 현재 이름으로 바뀝니다." : "알림은 선택사항이에요. iPhone은 홈 화면에 추가 후 이용해주세요.")}</p></section>;
}
