...
function Participants({ data, mutate }: { data: AdminData; mutate: (path: string, method: "POST" | "PATCH" | "DELETE", body: unknown, success: string) => Promise<boolean> }) {
  ...
  async function add(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await mutate("/api/admin/participants", "POST", { challengeId: challenge!.id, name: form.get("name"), affiliation: form.get("affiliation"), joinedAt: form.get("joinedAt"), paidAmount: Number(form.get("paidAmount")), paidAt: form.get("paidAt") || null }, "참가자를 추가했습니다.");
    if (ok) element.reset();
  }
  ...
  async function toggle(person: AdminParticipant) {
    ...
    await mutate(`/api/admin/participants/${person.id}`, "PATCH", { challengeId: challenge!.id, name: person.name, affiliation: person.affiliation, joinedAt: person.joinedAt, leftAt: person.leftAt, paidAmount: person.paidAmount, paidAt: person.paidAt, isActive: !person.isActive }, "참가 상태를 변경했습니다.");
  }
  async function confirmPayment(person: AdminParticipant) {
    const amount = person.paidAmount > 0 ? person.paidAmount : challenge!.defaultFee;
    if (!window.confirm(`${person.name} 참가자의 ${won(amount)} 납부를 확인 처리할까요?`)) return;
    await mutate(`/api/admin/participants/${person.id}`, "PATCH", { challengeId: challenge!.id, name: person.name, affiliation: person.affiliation, joinedAt: person.joinedAt, leftAt: person.leftAt, paidAmount: amount, paidAt: kstDateKey(), isActive: person.isActive }, "납부를 확인했습니다.");
  }
  const visibleParticipants = data.participants.filter((person) => {
    if (paymentFilter === "paid") return person.paymentStatus === "paid";
    if (paymentFilter === "outstanding") return person.paymentStatus === "unpaid" || person.paymentStatus === "unconfirmed";
    return true;
  });
  return <div className="grid gap-5">
    <form onSubmit={add} className="surface-card grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><label className="grid gap-1 text-sm font-bold">이름<input className="form-input" name="name" placeholder="참가자 이름" required /></label><label className="grid gap-1 text-sm font-bold">소속 (선택)<input className="form-input" name="affiliation" maxLength={120} /></label><label className="grid gap-1 text-sm font-bold">참여일<input className="form-input" name="joinedAt" type="date" defaultValue={challenge.startDate} required /></label><label className="grid gap-1 text-sm font-bold">실제 납부액<input className="form-input" name="paidAmount" type="number" min="0" defaultValue={challenge.defaultFee} required /></label><label className="grid gap-1 text-sm font-bold">납부 확인일<input className="form-input" name="paidAt" type="date" max={kstDateKey()} /><span className="text-xs font-normal text-slate-500">아직 납부하지 않았다면 비워 두세요.</span></label><div className="self-end"><SubmitButton>참가자 추가</SubmitButton></div></form>
    <div className="flex flex-wrap items-center gap-2">
      {([["all", "전체"], ["paid", "납부 완료"], ["outstanding", "미납·확인 필요"]] as Array<[PaymentFilter, string]>).map(([value, label]) => (
        <button key={value} type="button" className={`filter-button ${paymentFilter === value ? "filter-active" : ""}`} aria-pressed={paymentFilter === value} onClick={() => setPaymentFilter(value)}>{label}</button>
      ))}
      <span className="text-xs font-bold text-slate-500">납부 완료 {data.metrics.paidCount}명 · 미납 {data.metrics.unpaidCount}명</span>
    </div>
    <section className="surface-card !p-0 overflow-hidden"><div className="overflow-x-auto"><table className="min-w-[1000px] w-full text-sm"><thead className="bg-slate-50 text-left text-xs text-slate-500"><tr>{["이름", "참여 기간", "완료", "미완료", "링크", "납부", "벌금", "환급액 / 상태", "관리"].map((item) => <th className="px-4 py-3" key={item}>{item}</th>)}</tr></thead><tbody>{visibleParticipants.map((person) => <tr className={`border-t border-slate-100 ${person.isActive ? "" : "opacity-50"}`} key={person.id}><td className="px-4 py-3 font-black">{person.name}</td><td className="px-4 py-3">{person.joinedAt} ~ {person.leftAt ?? "참여 중"}</td><td className="px-4 py-3 text-emerald-700 font-bold">{person.completedDays}</td><td className="px-4 py-3 text-rose-700 font-bold">{person.missedDays}</td><td className="px-4 py-3">{person.totalLinks}</td><td className="px-4 py-3"><span className="font-bold">{won(person.paidAmount)}</span><span className={`mt-1 inline-block rounded-full px-2.5 py-1 text-xs font-black ${paymentTones[person.paymentStatus]}`}>{paymentLabels[person.paymentStatus]}</span>{person.paidAt && <span className="mt-1 block text-xs text-slate-500">{person.paidAt} 확인</span>}</td><td className="px-4 py-3">{won(person.penaltyAmount)}</td><td className="px-4 py-3 font-black">{won(person.refundedAmount ?? person.expectedRefund)}<span className={`mt-1 block text-xs ${person.refundedAmount !== null ? "text-emerald-700" : "text-slate-500"}`}>{person.refundedAmount !== null ? "환급 완료" : "예상 환급"}</span>{(person.paymentStatus === "unpaid" || person.paymentStatus === "unconfirmed") && <span className="mt-1 block text-xs font-bold text-rose-600">납부 대기</span>}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1"><button type="button" className="icon-button !h-9 !w-9" aria-label={`${person.name} 수정`} onClick={() => setEditing(person)}><Pencil size={15} /></button><button type="button" className="filter-button !min-h-9" onClick={() => void toggle(person)}>{person.isActive ? "비활성" : "활성"}</button>{(person.paymentStatus === "unpaid" || person.paymentStatus === "unconfirmed") && <button type="button" className="filter-button !min-h-9" onClick={() => void confirmPayment(person)}>납부 확인</button>}</div></td></tr>)}</tbody></table></div>{visibleParticipants.length === 0 && <p className="px-4 py-8 text-center text-sm font-bold text-slate-500">조건에 맞는 참가자가 없습니다.</p>}</section>
      ...
      <section aria-labelledby="participant-edit-title" aria-modal="true" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl" role="dialog">
        ...
        <form className="grid gap-4" onSubmit={save}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="grid gap-1.5 text-sm font-bold">납부 확인일 (미납이면 비움)<input className="form-input" name="paidAt" type="date" defaultValue={editing.paidAt ?? ""} max={kstDateKey()} /><span className="text-xs font-normal text-slate-500">현재 상태: {paymentLabels[editing.paymentStatus]}</span></label>
          </div>
...
function SettingsPanel({ data, mutate }: { data: AdminData; mutate: (path: string, method: "POST" | "PATCH" | "DELETE", body: unknown, success: string) => Promise<boolean> }) {
  ...
  async function editChallenge() {
    ...
    const penaltyStartDate = window.prompt("차감 시작일 (YYYY-MM-DD)", challenge.penaltyStartDate); if (!penaltyStartDate) return;
    ...
    await mutate(`/api/admin/challenges/${challenge.id}`, "PATCH", { name, startDate, endDate, penaltyStartDate, defaultFee: Number(defaultFee), defaultPenalty: Number(defaultPenalty) }, "기수 정보를 수정했습니다.");
  }
  ...
  return <div className="grid gap-5 xl:grid-cols-2">
    <SettingCard title="기수 만들기" description="새 기수는 생성 후 ‘운영 기수로 전환’을 눌러야 참가자 화면에 표시됩니다. 비활성 상태이며 운영 데이터가 없는 기수만 삭제할 수 있습니다."><form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => void submitForm(event, "/api/admin/challenges", (f) => ({ name: f.get("name"), startDate: f.get("startDate"), endDate: f.get("endDate"), penaltyStartDate: f.get("penaltyStartDate") || undefined, defaultFee: Number(f.get("defaultFee")), defaultPenalty: Number(f.get("defaultPenalty")) }), "기수를 만들었습니다.")}><input className="form-input sm:col-span-2" name="name" placeholder="기수 이름" required /><label className="grid gap-1 text-sm font-bold">시작일<input className="form-input" type="date" name="startDate" required /></label><label className="grid gap-1 text-sm font-bold">종료일<input className="form-input" type="date" name="endDate" required /></label><label className="grid gap-1 text-sm font-bold sm:col-span-2">차감 시작일<input className="form-input" type="date" name="penaltyStartDate" /><span className="text-xs font-normal text-slate-500">비우면 시작일과 같습니다. 이 날짜 이전 평일은 미제출로 집계하지 않습니다.</span></label><input className="form-input" type="number" min="0" name="defaultFee" defaultValue="80000" placeholder="기본 회비" required /><input className="form-input" type="number" min="0" name="defaultPenalty" defaultValue="2000" placeholder="기본 벌금" required /><div className="sm:col-span-2"><SubmitButton>기수 만들기</SubmitButton></div></form>{challenge && <div className="mt-3 grid gap-2 sm:grid-cols-2"><button className="secondary-button" onClick={() => void editChallenge()}><Pencil size={17} />현재 기수 수정</button>{!challenge.isActive && <><button className="secondary-button" onClick={() => { if (window.confirm(`${challenge.name}을 운영 기수로 전환할까요?`)) void mutate(`/api/admin/challenges/${challenge.id}/activate`, "POST", {}, "운영 기수를 전환했습니다."); }}><Check size={17} />운영 기수로 전환</button><button className="secondary-button !text-rose-600 sm:col-span-2" onClick={() => void deleteChallenge()}><Trash2 size={17} />현재 기수 삭제</button></>}</div>}</SettingCard>
